...
return (
  <div className="projectSlider">
    ...
    {projects.length > 1 && (
      <div className="projectSlider--dots">
        {projects.map((project, index) => (
          <button
            key={index}
            className={`projectSlider--dots__dot ${index === currentProject ? "active" : ""}`}
            onClick={() => setCurrentProject(index)}
            onMouseEnter={handleHover}
            onMouseLeave={handleLeave}
          >
            {isHovered && <span className="projectSlider--dots__dot--tooltip">{project.title}</span>}
          </button>
        ))}
  </div>
